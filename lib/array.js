Object.defineProperties(Array.prototype, {
  ...
  clear: { get: function() { this.length = 0; return this; }, configurable: true, enumerable: false },
  ...
  sum: { get: function() { return this.reduce((sum, x) => sum + x); }, configurable: true, enumerable: false },
  ...
  transpose: { 
      get: function() {
          return this.length === 0 ? [] : this[0].map((_, i) => this.map(row => row[i]));
      },
      configurable: true, enumerable: false 
  },
  tally: { 
...
Array.prototype.each_cons = function(n,func) {
...
Array.prototype.intersection = function(...arrs) {
  return [...new Set(this.filter(value => arrs.every(array => array.includes(value))))];
};

Array.prototype.difference = function(...arrs) {
  return [...new Set(this.filter(value => arrs.every(array => !array.includes(value))))];
};

Array.prototype.delete_at = function(n) {
  if (n < -this.length || n >= this.length) return undefined;
  return this.splice(n < 0 ? this.length + n : n, 1)[0];
};

Array.prototype.dig = function(...indices) {
...
Array.prototype.drop = Array.prototype.slice;